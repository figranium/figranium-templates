...
function PresetIcon({ icon }: { icon: string }) {
    ...
    if (!isDomain) {
        return <TablerIcon name={icon || "public"} className="text-3xl text-foreground" />;
    }

    if (error || !icon) {
        return <TablerIcon name="public" className="text-3xl text-foreground" />;
    }
    ...
    return (
        <div className="flex items-center gap-2 mt-1">
            <div className="w-6 h-6 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-[10px] font-semibold border border-[#262626] bg-[#121212] text-muted-foreground">
                {finalProfilePicture ? <img src={finalProfilePicture} alt="" className="w-full h-full object-cover" /> : initial}
            </div>
    ...
    return (
        ...
        >
            <div className="flex justify-between items-start mb-4">
                ...
                <span className="rounded-[7px] border border-white/[0.085] bg-black/30 px-2 py-1 font-mono text-[9px] text-white/42">{type}</span>
            </div>

            <p className="mb-6 line-clamp-3 text-[13px] leading-6 text-white/42">{description}</p>

            <div className="mt-auto flex items-center">
                <div className="flex items-center font-mono text-[10px] text-white/32">
                    <div className="flex items-center gap-1.5">
                        <TablerIcon name="download" className="text-xs" />
                        <span>{downloads}</span>
                    </div>
                </div>
            </div>
